<div>
	<div>
		<h3><?php echo __('Delivery Orders'); ?>
		<?php echo $this->Form->create('DeliveryOrder', array('url' => array_merge(array('action' => 'index'), $this->params['pass']),'class'=>'navbar-search pull-right')); ?>
		...
		<div class="filter-box" style="display:none">
			<?php echo $this->Form->create('DeliveryOrder', array('url' => array_merge(array('controller'=>'delivery_order','action' => 'index'), $this->params['pass']),'class'=>'form-inline')); ?>
			<fieldset>
	...
	<table class="table table-bordered table-hover table-striped table-striped-warning">
		...
		<tr>
			<td><?php echo $delivery_order['DeliveryOrder']['number']?></td>
			...
			<td><?php echo $delivery_order['DeliveryOrder']['order_date']; ?></td>
			<td>
				<a href="<?php echo $this->Html->url(array('controller' => 'delivery_order', 'action' => 'view', $delivery_order['DeliveryOrder']['id'])); ?>"><i class="icon-eye-open"></i></a>
			</td>
		</tr>
		<?php endforeach;?>
		</tbody>
...
jQuery(function($) {
	...
	$('#filter-result-close').click(function(){
		window.location.href = "<?php echo $this->Html->url(array_merge(array('controller'=>'delivery_order','action' => 'index'), $this->params['pass'])); ?>";
	});
});
</script>