<div>
	<?php echo $this->Form->create('PurchaseOrder',array('inputDefaults'=>array('label'=>false,'div'=>false))); ?>
	<div class="row-fluid">
			...
			<div class="row-fluid">
				<div class="span12">
					<table class="table table-condensed">
						<thead>
						<tr>
							<th style="vertical-align: top"><?php echo __('Number:'); ?></th>
							<th><?php echo $this->Form->input('number',array('type'=>'text'))?></th>
						</tr>
						</thead>
						<tr>
							<td><?php echo __('Supplier:'); ?></td>
							<td><?php echo $this->Form->input('supplier_id',array('data'=>$suppliers,'type'=>'select'))?></td>
						</tr>
						<tr>
			...
			<div class="tabbable contact-tabbable">
				<ul class="nav nav-tabs">
			  		...
			  		<div class="tab-pane active" id="tab1">
			  			<table class="table table-bordered table-hover table-striped table-striped-success">
							...
							<tbody id="record-tbody">
								<?php if(@$this->data['PurchaseOrder']['items']):?>
									<?php foreach($this->data['PurchaseOrder']['items'] as $k=>$item):?>
									<tr class="native-record">
										<td><span class="removerow" style="cursor: pointer"><i class="icon-remove"></i></span></td>
										<td data-modal-title="<?php echo __('Add Item')?>" data-modal-href="<?php echo Router::url(array('controller'=>'PurchaseOrder','action'=>'addPart'))?>" class="native-row">
											<?php echo $this->Form->input("PurchaseOrder.items.$k.id",array('type'=>'hidden','class'=>'span12 m-wrap part_id'))?>
											<span class="text"><?php echo $goods[$item['id']]?></span>
										</td>
										<td><?php echo $this->Form->input("PurchaseOrder.items.$k.description",array('class'=>'span12 m-wrap description'))?></td>
										<td class="control-group controls"><?php echo $this->Form->input("PurchaseOrder.items.$k.price",array('class'=>'right_align span12 m-wrap number price'))?></td>
										<td class="control-group controls"><?php echo $this->Form->input("PurchaseOrder.items.$k.quantity",array('class'=>'right_align span12 m-wrap number quantity'))?></td>
										<td class="control-group controls"><?php echo $this->Form->input("PurchaseOrder.items.$k.total",array('class'=>'right_align span12 m-wrap number totalprice'))?></td>
									</tr>
									<?php endforeach;?>
								<?php endif;?>
							</tbody>
			  	...
			  	<?php if(isset($this->data['PurchaseOrder']['id'])):?>
					<?php echo $this->Html->link(__('Back'),array('controller'=>'purchase_order','action'=>'view',$this->data['PurchaseOrder']['id']),array('class'=>'btn btn-inverse'));?>
				<?php else:?>
...
<?php if(@$this->data['PurchaseOrder']['items']):?>
var i = <?php echo count($this->data['PurchaseOrder']['items'])?>;
<?php else:?>
var i = 0;
<?php endif;?>
function add(){
	...
	var url = "<?php echo Router::url(array('controller'=>'Good','action'=>'find'))?>/"+$('#PurchaseOrderItemsId').val();
	 $.ajax({
		 url:url,
		 type:'get',
		 dataType:'json'
