...
class DealsController extends AppController {
	...
	public function add(){
		...
		if(empty($contacts)) {
			...
			$this->Session->setFlash(__('Please add contacts first. To add contact ') . $add_contact_link,'alert',array('class'=>'alert-info'));
		}
		...
		if($this->request->is('post')){
			...
			if($this->Deal->save($this->request->data)){
				$this->Session->setFlash(__('New deal added'),'alert');
	...
	public function edit($id = null){
		...
		if ($user_id['Deal']['user_id'] != $this->Auth->user('id')) {
			$this->Session->setFlash(__('Oops! Looks like you are not the owner of Deal. Only the user corresponding to the deal can edit it.'),'alert',array('class'=>'alert-error'));
			$this->redirect(array('action'=>'index'));
		}
		if($this->request->is('post')){
			if($this->Deal->save($this->request->data)){
				$this->Session->setFlash(__('Deal saved'),'alert');
				$this->redirect(array('action'=>'index'));
			}
			else
			{
	...
	public function delete($id = null){
		...
		if ($user_id['Deal']['user_id'] == $this->Auth->user('id')){
			if($this->Deal->delete($id)){
				$this->Session->setFlash(__('Deal deleted'),'alert');
				$this->redirect(array('action'=>'index'));
			}
			else
			{
